...
void StateMachineEditor::remove_link(int id)
{
...
void StateMachineEditor::drawInterface()
{
    ...
    for (auto &elem : states)
    {
        ...
        ImGui::Spacing();
        {
            ...
            {
                ImGui::Text("%s", elem.second.transitions[i]->className().c_str());
            }
        ...
        if (strcmp(elem.second.nameBuffer, ""))
        {
            if (ImGui::Button("update"))
            {
                ...
                std::shared_ptr<game::StateMachine::State> state = std::make_shared<game::StateMachine::State>(*editorTarget(), name);
